...
var JustLog, MIN_ROTATE_MS, create, cwd, debug, defaultLogFile, error, events, fs, info, k, levels, mkdirp, moment, os, path, pattern, timeout, util, v, warn, _ref,
  ...
  */
  ...
  function JustLog(options) {
    ...
    if (this.options.stdio) {
      ...
      this.options.stdio.render = pattern.compile((_ref1 = pattern.pre[this.options.stdio.pattern]) != null ? _ref1 : this.options.stdio.pattern);
    }
    if (this.options.file) {
      this.options.file.render = pattern.compile((_ref2 = pattern.pre[this.options.file.pattern]) != null ? _ref2 : this.options.file.pattern);
      this._initFile();
    }
  }
